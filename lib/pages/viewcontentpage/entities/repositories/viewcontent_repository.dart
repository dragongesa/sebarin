...
class ViewContentRequest {
  static getUserEventInfo(Event event) async {
    LoginDetails loginDetails = await Picker.getLoginDetails();
    int? userId = loginDetails.userId;
    int eventId = event.id;
    final url = "/api/events/is_attend.php?eid=$eventId&uid=$userId";
    Response response;
    var dio = initiateDio();
  ...
  static attendEvent(FormData data) async {
    final url = "/api/events/attend.php";
    Response response;
    Dio dio = initiateDio();
    dio.interceptors.add(InterceptorsWrapper(
      onError: (e, handler) {
